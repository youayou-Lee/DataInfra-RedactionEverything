...
归类补充（2026-09-15 评审定稿）：execute 阶段 5xx（打码服务崩溃）与化名对照缺失归 **soft**（可重跑裁决，单次偶发不升级档位）；任何一路残留检测命中（本地 grep / execute 自检 / API 复扫）与复扫本身执行失败均归 **hard**；含 ERROR 未重跑的格式结论为「待定」，不给绿灯。
